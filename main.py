...
def redirector(request: Request):
    ...
    doc_ref = _db.collection('links').document(link_id)
    snap = doc_ref.get()
    if not snap.exists:
    ...
    # Prepare counters + optional business aggregate in a batch
    try:
        batch = _db.batch()
        ...
        if business_id:
            biz_ref = _db.collection('businesses').document(str(business_id))
            base = {}
            if data.get('business_name'):
                base['name'] = data['business_name']
            base.update({
                'hit_count': firestore.Increment(1),
                'last_hit_at': firestore.SERVER_TIMESTAMP,
            })
            batch.set(biz_ref, base, merge=True)

        batch.commit()
    except Exception:
        # Keep redirecting even if counters fail
        pass

    # Per-hit log (detailed analytics)
    try:
        ...
        # Extract client IP (first in X-Forwarded-For)
        ...
        # Optional geo lookup (derived only; no raw IP persisted)
        print("Client IP from X-Forwarded-For:", client_ip)
        if client_ip and not _is_private_ip(client_ip):
            geo = _lookup_geo(client_ip)
            print("Main function geo", geo)
            if geo:
        ...
        #if HIT_TTL_DAYS > 0:
        #    hit['expires_at'] = datetime.now(timezone.utc) + timedelta(days=HIT_TTL_DAYS)
